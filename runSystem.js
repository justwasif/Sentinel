 ...
 * ────────────
 * Starts the full Sentinel swarm:
 *   1. Swarm Coordinator (listens for proposals, owns the full proof lifecycle)
 *   2. Yield Agent (monitors LP tick range)
 *   3. Risk Agent  (monitors lending health factors)
 *
 * The coordinator handles submitProof → isProofValid → execute → consumeProof
 * internally via NonceManager.  Do NOT submit proofs here — that was the
 * source of the nonce collision.
 *
 * Usage:
 *   node runSystem.js
 */
...
async function start() {
  ...
  // Start all three processes.
  // Proof submission is handled exclusively inside the coordinator — no
  // external submitProof calls here to avoid nonce races.
  coordinator.runCoordinator();
  riskAgent.main();
  await yieldAgent.main();
